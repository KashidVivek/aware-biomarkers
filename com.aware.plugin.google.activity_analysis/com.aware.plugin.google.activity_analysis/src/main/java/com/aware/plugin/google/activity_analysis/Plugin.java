...
public class Plugin extends Aware_Plugin implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public void onCreate() {
        ...
        if (!is_google_services_available()) {
            ...
            Intent gARIntent = new Intent(getApplicationContext(), com.aware.plugin.google.activity_analysis.Algorithm.class);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (PERMISSIONS_OK) {
            ...
            if (intent != null && intent.getAction() != null && intent.getAction().equalsIgnoreCase(ACTION_AWARE_ACTIVITY_ANALYSIS)) {
                ...
                context_data.put(Google_AR_Provider.ActivityAnalysisData.TIMESTAMP, System.currentTimeMillis());
                context_data.put(Google_AR_Provider.ActivityAnalysisData.DEVICE_ID, Aware.getSetting(getApplicationContext(), Aware_Preferences.DEVICE_ID));
                context_data.put(Google_AR_Provider.ActivityAnalysisData.MODERATE_ACTIVITY_TIME, stats.getModerateActivityTime());
                context_data.put(Google_AR_Provider.ActivityAnalysisData.HIGH_ACTIVITY_TIME, stats.getHighActivityTime());
                context_data.put(Google_AR_Provider.ActivityAnalysisData.IS_USER_ACTIVE, stats.isUserActiveInDay());

                getContentResolver().insert(Google_AR_Provider.ActivityAnalysisData.CONTENT_URI, context_data);

//                Intent context = new Intent(ACTION_AWARE_ACTIVITY_ANALYSIS);
//                context.putExtra(Provider.ActivityAnalysisData.MODERATE_ACTIVITY_TIME, stats.getModerateActivityTime());
//                context.putExtra(Provider.ActivityAnalysisData.HIGH_ACTIVITY_TIME, stats.getHighActivityTime());
//                context.putExtra(Provider.ActivityAnalysisData.IS_USER_ACTIVE, stats.isUserActiveInDay());
//                sendBroadcast(context);
            ...
            try {
                ...
                if (contacts_sync == null || contacts_sync.getInterval() != Long.parseLong(Aware.getSetting(this, Settings.FREQUENCY_PLUGIN_ACTIVITY_ANALYSIS))) {
                    contacts_sync = new Scheduler.Schedule(SCHEDULER_PLUGIN_ACTIVITY_ANALYSIS);
//                    contacts_sync.setInterval(Integer.parseInt(Aware.getSetting(this, Settings.FREQUENCY_PLUGIN_ACTIVITY_ANALYSIS)) * 60 * 24);
    ...
    public void onConnected(Bundle bundle) {
        ...
        ActivityRecognition.ActivityRecognitionApi.requestActivityUpdates(gARClient,
                Long.valueOf(Aware.getSetting(getApplicationContext(), Settings.FREQUENCY_PLUGIN_GOOGLE_ACTIVITY_RECOGNITION)) * 1000, gARPending);
    }
